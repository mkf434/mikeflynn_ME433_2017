...
#define XL_MSG 0x82             // Sample rate to 1.66KHz, 2g sensitive, 100Hz filter
#define CTRL2_G  0x11           // Turn on gyroscope
#define G_MSG 0x88              // Sample rate to 1.66KHz, 1000 dps sensitivity
#define CTRL3_C  0x12           // contains IF_INC bit = 1, set by default
#define C_MSG 0x04              // might as well check it
...
void initPololu(){                    // Set GP0-3 as outputs and GP4-7 as inputs
  ...
  sendMessage(CTRL1_XL, XL_MSG);      // Turn on and init accelerometer
  sendMessage(CTRL2_G,G_MSG);         // Turn on and init gyroscope
  sendMessage(CTRL3_C,C_MSG);
